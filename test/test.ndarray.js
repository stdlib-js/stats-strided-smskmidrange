...
tape( 'the function has an arity of 7', function test( t ) {
	t.strictEqual( smskmidrange.length, 7, 'has expected arity' );
	t.end();
});
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `NaN`', function test( t ) {
	...
	v = smskmidrange( 0, x, 1, 0, mask, 1, 0 );
	t.strictEqual( isnanf( v ), true, 'returns expected value' );

	v = smskmidrange( -1, x, 1, 0, mask, 1, 0 );
	t.strictEqual( isnanf( v ), true, 'returns expected value' );
...
tape( 'the function supports `stride` parameters', function test( t ) {
	...
	v = smskmidrange( 5, x, 2, 0, mask, 2, 0 );
...
tape( 'the function supports negative `stride` parameters', function test( t ) {
	...
	v = smskmidrange( 5, x, -2, 8, mask, -2, 8 );
...
tape( 'the function supports `offset` parameters', function test( t ) {
	...
	v = smskmidrange( 5, x, 2, 1, mask, 2, 1 );
	t.strictEqual( v, 1.0, 'returns expected value' );
